<template>
  <div class="post-detail">
    <div class="container-mobile section-spacing">
      <!-- Header -->
      <div class="mb-6">
        <button @click="$router.back()" class="btn-secondary btn-touch mb-4">
      ...
      <article v-else-if="post" class="post-content">
        ...
        <!-- Post Body -->
        <div
          class="post-body prose prose-sm max-w-none"
        ...
        <footer class="mt-8 pt-6 border-t border-gray-200">
          <div class="flex gap-2">
            ...
            <button @click="sharePost" class="btn-secondary btn-touch">
...
// DOMPurify configuration for consistent and performant sanitization
const PURIFY_CONFIG = {
  ALLOWED_TAGS: [
    "p",
    "br",
...
const sanitizedDescription = computed(() => {
  ...
  return DOMPurify.sanitize(post.value.description, PURIFY_CONFIG);
});
...
async function loadPost() {
  ...
  try {
    ...
    const loadedPost = {
      ...
      title: "샘플 게시글 제목",
      description:
        "<p>게시글 내용입니다. 실제 구현에서는 API에서 데이터를 가져와야 합니다.</p>",
    ...
    const bookmarks = JSON.parse(
      localStorage.getItem("knue-bookmarks") || "[]"
    );
  ...
  if (navigator.share && post.value) {
    try {
      await navigator.share({
        ...
        text: DOMPurify.sanitize(post.value.description, {
          ALLOWED_TAGS: [],
        }).substring(0, 200),
