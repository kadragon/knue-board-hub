    ...
    <div v-if="expanded" class="debugger-content">
      ...
      <div class="stats-section">
        ...
        <div class="categories">
          <h5>By Category:</h5>
          <div v-for="(category, name) in cacheStats.categories" :key="name" class="category-item">
            <span class="category-name">{{ name }}:</span>
            <span class="category-stats">{{ category.count }} items, {{ formatBytes(category.size) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="departments-section">
        ...
        <div class="cache-info">
          <div class="info-item">
            ...
            <span class="info-value">{{ departmentStats.departmentsLoaded }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">Initialized:</span>
            <span class="info-value">{{ departmentStats.isInitialized ? '✅' : '❌' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">Last Fetch:</span>
            <span class="info-value">{{ formatTime(departmentStats.lastFetch) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="rss-section">
        ...
        <div class="cache-info">
          ...
          <div class="info-item">
            ...
            <span class="info-value">{{ formatTime(rssStats.lastUpdate) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="rehydration-section">
        ...
        <div class="cache-info">
          <div class="info-item">
            ...
            <span class="info-value">{{ rehydrationStats.queuedRehydrations }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">Background Tasks:</span>
            <span class="info-value">{{ rehydrationStats.backgroundTasks }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">Pending Requests:</span>
            <span class="info-value">{{ rehydrationStats.pendingRequests }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">Processing:</span>
            <span class="info-value">{{ rehydrationStats.isProcessingQueue ? '🔄' : '⏸️' }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="actions-section">
        <button @click="refreshStats" class="action-btn">🔄 Refresh Stats</button>
        <button @click="clearCache" class="action-btn danger">🗑️ Clear Cache</button>
        <button @click="runPerformanceTest" class="action-btn">⚡ Performance Test</button>
        <button @click="exportCacheData" class="action-btn">💾 Export Data</button>
      </div>
      ...
      <div v-if="performanceResults" class="performance-section">
        ...
        <div class="performance-results">
          <div class="result-item">
            ...
            <span class="result-value">{{ performanceResults.writeSpeed }}ms/op</span>
          </div>
          <div class="result-item">
            <span class="result-label">Read Speed:</span>
            <span class="result-value">{{ performanceResults.readSpeed }}ms/op</span>
          </div>
          <div class="result-item">
            <span class="result-label">Operations:</span>
            <span class="result-value">{{ performanceResults.operations }}</span>
          </div>
        </div>
      </div>
...
const runPerformanceTest = async () => {
  ...
  try {
    ...
    addLog(`⚡ Performance test completed: ${performanceResults.value.writeSpeed}ms write, ${performanceResults.value.readSpeed}ms read`, 'success')
  } catch (error) {
...
const watchStorageChanges = () => {
  ...
  localStorage.setItem = function(key, value) {
    if (key.startsWith('knue-board-hub:')) {
      addLog(`💾 Cache SET: ${key.replace('knue-board-hub:', '')}`, 'cache')
    }
  ...
  localStorage.removeItem = function(key) {
    if (key.startsWith('knue-board-hub:')) {
      addLog(`🗑️ Cache DELETE: ${key.replace('knue-board-hub:', '')}`, 'cache')
    }
...
.stat-item, .info-item, .result-item {
  display: flex;
  justify-content: space-between;
  padding: 4px 0;
}

.stat-label, .info-label, .result-label {
  color: #ccc;
}

.stat-value, .info-value, .result-value {
  font-weight: bold;
  color: #4ade80;
}
...
.departments-section, .rss-section, .rehydration-section, .performance-section {
  margin-top: 16px;
  padding-top: 8px;
  border-top: 1px solid #333;
}

.departments-section h4, .rss-section h4, .rehydration-section h4, .performance-section h4 {
  margin: 0 0 8px 0;
  font-size: 12px;
  color: #60a5fa;
}
